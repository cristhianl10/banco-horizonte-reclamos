...
public sealed class ComplaintsController(AppDbContext db, ComplaintService service, IPriorityAndSlaService calculator, TimeProvider clock) : ControllerBase
{
    ...
    {
        if (query.Page < 1 || query.PageSize is < 1 or > 100) return BadRequest(new ProblemDetails { Title = "Paginación inválida", Detail = "Page debe ser >= 1 y PageSize debe estar entre 1 y 100." });
        var source = db.Complaints.AsNoTracking().Include(x => x.Customer).Include(x => x.Category).Include(x => x.Status)
            .Include(x => x.Priority).Include(x => x.CurrentAssignee).Include(x => x.SlaPolicy).AsQueryable();
        ...
        if (query.Sla?.Equals("overdue", StringComparison.OrdinalIgnoreCase) == true) source = source.Where(x => x.SlaDeadline <= now && !x.Status.IsFinal);
        if (query.Sla?.Equals("near", StringComparison.OrdinalIgnoreCase) == true) source = source.Where(x => x.SlaDeadline > now && x.SlaDeadline <= now.AddHours(4) && !x.Status.IsFinal);
        ...
        return Ok(new PagedResult<ComplaintListItem>(entities.Select(x => ComplaintService.MapListItem(x, now, calculator)).ToList(), query.Page, query.PageSize, total));
    }
    ...
    public async Task<ActionResult<ComplaintDetail>> Detail(Guid id, CancellationToken ct)
    {
        var x = await db.Complaints.AsNoTracking().Include(x => x.Customer).Include(x => x.ReceptionChannel).Include(x => x.Category)
            .Include(x => x.Subcategory).Include(x => x.Status).Include(x => x.Priority).Include(x => x.SlaPolicy).Include(x => x.CurrentAssignee)
        ...
        var timeline = x.History.Select(h => new TimelineItem(h.EventType, h.AfterData ?? "Sin detalle", $"{h.Actor.FirstNames} {h.Actor.LastNames}", h.OccurredAt))
            .Concat(x.Observations.Select(o => new TimelineItem("OBSERVACION", o.Content, $"{o.Author.FirstNames} {o.Author.LastNames}", o.CreatedAt)))
            .OrderByDescending(x => x.At).ToList();
        return Ok(new ComplaintDetail(x.Id, x.Code, $"{x.Customer.FirstNames} {x.Customer.LastNames}", $"{x.Customer.DocumentType} {x.Customer.DocumentNumber}",
            x.Customer.Email, x.Customer.Phone, x.ReceptionChannel.Name, x.Category.Name, x.Subcategory?.Name, x.Description, x.Impact, x.Urgency,
            x.Status.Name, x.StatusId, x.Priority.Name, x.PriorityScore, x.CurrentAssignee is null ? null : $"{x.CurrentAssignee.FirstNames} {x.CurrentAssignee.LastNames}",
            x.CurrentAssigneeId, x.ReceivedAt, x.SlaDeadline, calculator.GetSlaState(x.SlaDeadline, clock.GetUtcNow(), x.SlaPolicy.AlertThresholdMinutes), timeline));
    }
    ...
    public async Task<IActionResult> Create(CreateComplaintRequest request, CancellationToken ct)
    {
        ...
        if (result.Complaint is null) return Conflict(new { message = "Se encontraron posibles reclamos duplicados.", possibleDuplicates = result.Duplicates });
        return CreatedAtAction(nameof(Detail), new { id = result.Complaint.Id }, new { result.Complaint.Id, result.Complaint.Code, result.Complaint.SlaDeadline });
    }
    ...
    public async Task<IActionResult> Assign(Guid id, AssignComplaintRequest request, CancellationToken ct) { await service.AssignAsync(id, request, User.GetUserId(), ct); return NoContent(); }
